...
export default function NightDetective({name, players, knownRoles, setKnownRoles, sendMessageToParent}) {
        ...
        return (
            <>
                ...
                <Text size={42} weight={700}>
                    {playersToList.find(player => player.name === selectedPlayer.name).role}
                </Text>
                <Box style={{flex: 1}}/>
                <Text size={18} opacity={0.75}>
    ...
    return (
        <>
                    ...
                    playersToList.map((player, index) => {
                        return (
                            <NameButton
                                ...
                                disabled={knownRoles.includes(player.name) || player.name === name}
                            >
                                <Text size={18}>
                                    {player.realName}{knownRoles.includes(player.name) ? ` (${player.role})` : ''}
                                </Text>
                            </NameButton>
                        )
                    })
            ...
            <Box style={{flex: 1}}/>
            <TheButton disabled={selectedPlayer === null} onClick={handleConfirmation}>
                <Text size={18} weight={700}>
                    identify
                </Text>
            </TheButton>
