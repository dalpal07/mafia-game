...
import { ACCUSED, GAME_OVER, INSTRUCTIONS, NIGHTTIME, NIGHTTIME_TIMER, NIGHT_OVER, POST_STORY_2, REVEAL_IDENTITY, STORY, VOTING, VOTING_RESULTS, VOTING_TIMER, WELCOME, YOU_READY } from "../pages";
...
export const ActionProvider = ({ children }) => {
    ...
    const handleMafiaSelection = (mafiaGamername, targetGamername) => { 
        const mafiaPlayer = players.find(player => player.gamername === mafiaGamername);
        const targetPlayer = players.find(player => player.gamername === targetGamername);
        if (!mafiaPlayer || !targetPlayer) {
            return;
        }
        const currentMafiaSelection = currentMafiaSelections.find(selection => selection.player === mafiaPlayer);
        // if mafia member has not selected a target, add the target to the currentMafiaSelections array
        if (!currentMafiaSelection) {
            setCurrentMafiaSelections([...currentMafiaSelections, { player: mafiaPlayer, target: targetPlayer }]);
        ...
        // if mafia member has selected a target and this target is different from the previous target, update the target in the currentMafiaSelections array
            ...
            setCurrentMafiaSelections(currentMafiaSelections.map(selection => {
        ...
        // if mafia member has selected a target and this target is the same as the previous target, remove the target from the currentMafiaSelections array
            ...
            setCurrentMafiaSelections(currentMafiaSelections.filter(selection => selection.player !== mafiaPlayer));
        }
    };
    const handleMafiaVote = (mafiaGamername, targetGamername) => {
        ...
        const currentMafiaVote = currentMafiaVotes.find(selection => selection.player === mafiaPlayer);
        // if mafia member has not voted, add the vote to the currentMafiaVotes array
        if (!currentMafiaVote) {
            setCurrentMafiaVotes([...currentMafiaVotes, { player: mafiaPlayer, target: targetPlayer }]);
        }
    ...
    const handleDetectiveIdentification = (targetGamername) => { 
        ...
        // find the target
        const target = players.find(player => player.gamername === targetGamername);
        // if the detective and target are found, add the identification to the detectiveIdentifications array
        if (!detective || !target) {
            return;
        }
        setDetectiveIdentifications([...detectiveIdentifications, { player: detective, target }]);
    ...
    const handleCivilianTriviaFinish = (gamername) => { 
        ...
        setCurrentCivilianTriviaFinishes([...currentCivilianTriviaFinishes, player]);
        // TODO: check if night is over
    };

    const handleAccusation = (accuserGamername, targetGamername) => { 
        // find the accuser
        const accuser = players.find(player => player.gamername === accuserGamername);
        // find the target
        const target = players.find(player => player.gamername === targetGamername);
    ...
    const handleLifeDeathSelection = (voterGamername, vote) => {
        ...
        const currentVote = currentLifeDeathSelections.find(vote => vote.player === voter);
        // if the voter has not voted, add the vote to the currentLifeDeathSelections array
        if (!currentVote) {
            setCurrentLifeDeathSelections([...currentLifeDeathSelections, { player: voter, vote }]);
        }
        // if the voter has voted and this vote is different from the previous vote, update the vote in the currentLifeDeathSelections array
            ...
            setCurrentLifeDeathSelections(currentLifeDeathSelections.map(playerVote => {
        ...
        // if the voter has voted and this vote is the same as the previous vote, remove the vote from the currentLifeDeathSelections array
            ...
            setCurrentLifeDeathSelections(currentLifeDeathSelections.filter(playerVote => playerVote.player !== voter));
        }
    };
    const handleLifeDeathVote = (voterGamername, vote) => { 
        ...
        // get the current vote
        const currentVote = currentLifeDeathVotes.find(vote => vote.player === voter);
        // if the voter has not voted, add the vote to the currentLifeDeathVotes array
        if (!currentVote) {
            setCurrentLifeDeathVotes([...currentLifeDeathVotes, { player: voter, vote }]);
        }
    ...
    const handleProgressToStoryPage = () => { 
        ...
        const playerToDie = players.find(player => player.gamername === playersWithMaxVotes[randomIndex]);
        if (!playerToDie) {
            return;
        }
    ...
    const handleTransitionFromVotingResultsPage = () => {
        ...
        const votesToLive = currentLifeDeathVotes.filter(vote => vote.vote === "live").length;
        const votesToDie = currentLifeDeathVotes.filter(vote => vote.vote === "die").length;
